...
ADMIN_IDS = [7125501771]  # 🔴 CHANGE TO YOUR TELEGRAM ID
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not_joined:
        await update.message.reply_photo(
            photo=WELCOME_PHOTO,
            caption="🔒 Please join ALL channels below to use this bot.",
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "verify_join":
        ...
        await query.message.reply_text(
            "✅ Verification successful!\n\n📸 You can now send a photo."
        )
        return

    # ENHANCEMENT BUTTONS
    if query.data.startswith("enh_"):
        not_joined = await check_membership(user.id, context)
        if not_joined:
            await query.message.reply_text(
